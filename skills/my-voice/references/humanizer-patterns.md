# Humanizer engine (embedded)

This is the AI-pattern detection and removal engine for the My Voice skill, folded in whole from the blader/humanizer repo (v2.7.0, MIT). The workflow in SKILL.md calls this file as the separate audit pass after every draft. Run every pattern below against the draft, then do the draft, audit, final loop and confirm the final rewrite has zero em dashes.
...
Packaged from the [blader/humanizer](https://github.com/blader/humanizer) skill (MIT licensed), version 2.7.0, with frontmatter adapted for this skills environment.
