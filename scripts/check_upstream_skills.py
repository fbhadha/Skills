...
Reads plugins/*/upstream.json: for each upstream repository, the pinned commit,
...
def main(argv: list[str]) -> int:
    ...
    manifests = sorted(ROOT.glob("plugins/*/upstream.json"))
    if not manifests:
        print("no plugins/*/upstream.json found")
        return 0
    problems: list[str] = []
