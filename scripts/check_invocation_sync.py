...
For every SKILL.md under plugins/*/skills/ and skills/:

  frontmatter `disable-model-invocation: true`
      <=>  agents/openai.yaml `policy.allow_implicit_invocation: false`

A skill under plugins/ must have an agents/openai.yaml (Codex reads it for the
display name and the policy). Exit code is non-zero on any mismatch.
...
def main() -> int:
    failures: list[str] = []
    skill_files = sorted(ROOT.glob("plugins/*/skills/*/SKILL.md")) + sorted(
