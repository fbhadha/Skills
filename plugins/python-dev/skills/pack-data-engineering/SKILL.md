...
A pack is a reference with a fixed shape (`plugins/python-dev/packs/TEMPLATE.md`): what selects it, the shapes it knows, the canonical repo to cite, the extra checks it turns on, the faults it looks for, and how it tests. `py-design` carries the general rules; this file carries what changes when the objects moving through the code are tables.
