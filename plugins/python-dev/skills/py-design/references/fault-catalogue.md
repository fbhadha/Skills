...
The faults LLM-written Python commits by default, each with its tell, the mechanism that catches it, and the fix. Tools catch the first column of each group; the reviewer catches the rest. When a fault has a tool, do not spend review words on it.

## Shape

| Fault | Tell | Caught by | Fix |
|---|---|---|---|
| The giant module | One file keeps growing because it is the one already open. | `file_size` gate (400 lines for modules, 150 for tests). | Split by shape: one module per domain concept, entry points at the package root. |
| The grab bag | `utils/`, `helpers/`, `common/`, `misc/`, or a file named after a category instead of a concept. | `banned_dirs` gate. | Move each function to the concept it serves. |
| The shallow module | A class or function that only forwards to the next one down. Delete it and the callers change nothing. | Reviewer: deletion test. | Inline it. |
...
| Fault | Tell | Caught by | Fix |
|---|---|---|---|
| Instructions to a model in the code | `# TODO: implement error handling`, `# Make sure to`, `# Step 1:`, `# You are`, prompt text pasted as comments, a docstring that starts "This function". | `prompt_comments` gate. | Delete, or say the why. ADK prompts live in `prompts.py` or a loaded file, never inline in logic. |
| Tutorial comments | `i += 1  # increment i` | Reviewer. | Delete. A comment states what the code cannot. |
| Swallowed exceptions | `except Exception: pass`, `except: return None`, a broad except inside an ADK tool. | ruff `BLE001`, `S110`, `E722`; `TRY` rules. | Catch the specific type you can handle; let the rest propagate; exceptions are part of the interface. |
...
| Duplication under a new name | A second `normalize_date` because the first was not in context. | Reviewer: search `CONTEXT.md` terms before writing a function; duplicate-code detection in `py-health`. | Reuse; if the two differ, name the difference. |
| Excessive complexity | Nested conditionals, 80-line functions, six parameters. | ruff `C901`, `PLR0912`, `PLR0913`, `PLR0915`. | Extract until each function reads as one sentence. |
...
| Fault | Tell | Caught by | Fix |
|---|---|---|---|
| The tautological test | The expected value is computed the way the code computes it. | Reviewer; mutation testing (`py-health`). | A literal from the spec or a worked example. |
| The instruction-shaped test | `def test_x(): pass  # TODO`, a body that is a comment, `assert True`. | `test_assertions` gate. | Write the behaviour or delete the test. |
| The mock-only test | Every collaborator mocked; the test proves the mocks were called. | Reviewer. | Test through the public interface with a real or in-memory adapter. |
| The weakened test | A loosened assertion, a deleted test, a new `skip`, a hard-coded return for the known input. | `weakened_tests` gate at commit; reviewer in a fresh context. | Tests are read-only from red to green. An intended deletion needs the explicit override and a reason in the commit. |
